...
| 功能 | 描述 |
|------|------|
| 👤 用户认证 | 完整的登录注册系统，JWT 令牌认证，密码 bcrypt 加密 |
...
| 🧠 思考过程展示 | 实时展示 AI 思考过程，提升透明度 |
| ⚡ 流式响应 | SSE 实时返回处理状态，提升用户体验 |
| 🔐 安全机制 | 只允许 SELECT 语句，密码 bcrypt 加密存储 |
| 💾 数据持久化 | MySQL 持久化存储用户、数据源、会话、消息 |
...
- Node.js >= 18.0 (前端开发)
- MySQL >= 8.0 (数据持久化)
- MySQL / PostgreSQL / SQLite 数据库 (业务数据源)
...
# - DB_HOST, DB_PORT, DB_NAME, DB_USERNAME, DB_PASSWORD (MySQL 配置)
# - SECRET_KEY (JWT 密钥)
...
- MySQL (aiomysql) - 系统数据库 + 业务数据源
- PostgreSQL (asyncpg) - 业务数据源
- SQLite (aiosqlite) - 业务数据源
...
│       └── agent_service.py      # DeepAgents 智能体
├── frontend/                     # 前端代码
│   ├── src/
│   │   ├── components/           # UI 组件
│   │   │   ├── Layout.tsx        # 布局组件
...
| datasources | 数据源表 | id, user_id, name, type, host, port, database_name |
| sessions | 会话表 | id, user_id, datasource_id, created_at, last_active_at |
...
| POST | `/api/sessions/{id}/messages/stream` | 发送消息 (SSE 流式) |
| DELETE | `/api/sessions/{id}/messages` | 清空会话消息 |
| GET | `/api/sessions/export/{downloadId}` | 下载导出文件 |
